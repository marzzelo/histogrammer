...
def build_html_report(
    ...
    total_weight, nbins, k, n_removed, png_path,
    show_cols=None,
):
    ...
    # ── stat cards ────────────────────────────────────────────────────────────
    time_card = (
        f'<div class="card"><div class="lbl">Total time</div>'
        f'<div class="val">{seconds_to_hhmmss(total_weight)}</div></div>'
    ) if has_time else ""
    outlier_card = (
        f'<div class="card"><div class="lbl">Outliers removed</div>'
        f'<div class="val">{n_removed}</div></div>'
    ) if k > 0 else ""
...
footer{{text-align:center;margin-top:2rem;font-size:.75rem;color:var(--mu)}}
</style>
</head>
<body>
<header>
...
<div class="cards">
  <div class="card"><div class="lbl">Samples</div><div class="val">{n_samples}</div></div>
  <div class="card"><div class="lbl">Min</div><div class="val">{v_min:.5g}</div></div>
  <div class="card"><div class="lbl">Max</div><div class="val">{v_max:.5g}</div></div>
  <div class="card"><div class="lbl">Mean</div><div class="val">{w_mean:.5g}</div></div>
  <div class="card"><div class="lbl">Std dev</div><div class="val">{w_std:.5g}</div></div>
  <div class="card"><div class="lbl">Median (P50)</div><div class="val">{p50:.5g}</div></div>
  <div class="card"><div class="lbl">P25 / P75</div><div class="val">{p25:.4g} / {p75:.4g}</div></div>
  {time_card}{outlier_card}
</div>
<div class="chart-wrap"><img src="{png_name}" alt="Histogram"></div>
<table>
  <thead>
    <tr>{grp_row}</tr>
    <tr>{sub_row}</tr>
...
<footer>Generated by histogram.py &nbsp;·&nbsp; {file_name}</footer>
</body>
</html>"""
...
def make_histogram(col, csv_path, time_col_arg, nbins, title, k=0.0, show_cols=None, out_name=None):
...
def run_gui():
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("SOGEFI — Histogram Generator")
        ...
        def _build_ui(self):
            ...
            self.w_out.setPlaceholderText("Optional — default: <csv>_hist_<col>.png/.html")
        ...
        def _run(self):
            ...
            try:
                ...
                self._set_status(
                    f"✔  Saved → {os.path.basename(png)}  |  {os.path.basename(html)}", "ok"
                )
...
def parse_cli(argv):
    ...
    for token in argv[3:]:
        ...
        if key == "t":
            ...
            sys.exit(f"Unknown keyword: {key!r}. Supported: t, bins, k, title, out.")

    return dict(col=col, csv_path=csv_path, time_col_arg=time_col_arg,
                nbins=nbins, title=title, k=k, out_name=out_name)
